...
// s is exhausted. If ctx is canceled before then, the goroutine stops
// draining s and closes the channel — cancel ctx when done with the
// channel if it might not be read to completion, to avoid leaking the
// goroutine.
